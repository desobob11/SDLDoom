...
{   int i = 0, j = 0;
    ...
    RVECTOR perp_dir = RVECTOR_rotate_vector(M_PI / 2, player->dir_vector);
    ...
    RVERTEX new_dir_left = {player->position.head.x + perp_x_incr, 0, player->position.head.z + perp_z_incr};
    RVERTEX new_dir_right = {player->position.head.x - perp_x_incr, 0, player->position.head.z - perp_z_incr};

        int check_left = new_dir_left.z - HORIZON_DIST  < map_vh && new_dir_left.x - HORIZON_DIST < map_vw
        && new_dir_left.z - HORIZON_DIST >= 0 && new_dir_left.x - HORIZON_DIST >= 0;

    int check_right = new_dir_right.z - HORIZON_DIST  < map_vh && new_dir_right.x - HORIZON_DIST < map_vw
        && new_dir_right.z - HORIZON_DIST >= 0 && new_dir_right.x - HORIZON_DIST >= 0;
