...
void AUTOMAP_render_wall(SDL_Surface* surface, NGIN::WALL wall,
    ...
    uint32_t x_off =
        wall.x_pos -
        player_pos.head.x;  // - AUTOMAP_get_draw_offset_x(player_pos);
    uint32_t z_off =
        wall.z_pos -
        player_pos.head.z;  // - AUTOMAP_get_draw_offset_z(player_pos);
...
void AUTOMAP_render_vector(SDL_Surface* surface, DOOM::Vector vector) {
    ...
    double vec_length = vector.Vector_length();
    ...
    for (int i = 0; i < vec_length; ++i) {
        ...
        if (x_draw >= 0 && z_draw >= 0 && x_draw < SCREEN_WIDTH &&
            z_draw < SCREEN_HEIGHT) {
...
uint32_t AUTOMAP_get_draw_offset_x(DOOM::Vector player_pos) {
    return player_pos.head.x -
           (player_pos.head.x - (SCREEN_WIDTH / 2) - (PLAYER_DOT_W / 2));
}

uint32_t AUTOMAP_get_draw_offset_z(DOOM::Vector player_pos) {
    return player_pos.head.z -
           (player_pos.head.z - (SCREEN_HEIGHT / 2) - (PLAYER_DOT_H / 2));
}
